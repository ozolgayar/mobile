...
const STORAGE_KEYS = {
  laptopScheduleDone: 'laptopScheduleDone',
  laptopGoalDone: 'laptopGoalDone',
  mapViewed: 'mapViewed',
  mod1Status: 'nc_mod1_status',
  mod1Progress: 'nc_mod1_progress'
};
...
function showScreen(id) {
  ...
  if (id === 'screen-10') {
    ...
    setTimeout(function() {
      updateLaptopProgressUI();
    }, 50);
  }

  if (id === 'screen-final') {
  localStorage.setItem(STORAGE_KEYS.mod1Status, 'complete');
  localStorage.setItem(STORAGE_KEYS.mod1Progress, '100');
...
function getLaptopCompletionState() {
  var scheduleDone = localStorage.getItem(STORAGE_KEYS.laptopScheduleDone) === '1';
  var goalDone = localStorage.getItem(STORAGE_KEYS.laptopGoalDone) === '1';
  var mapDone = localStorage.getItem(STORAGE_KEYS.mapViewed) === '1';
  return {
    scheduleDone: scheduleDone,
    goalDone: goalDone,
    mapDone: mapDone,
    allDone: scheduleDone && goalDone && mapDone
  };
}

function updateLaptopProgressUI() {
  var completion = getLaptopCompletionState();
  ...
  if (badgeSchedule) badgeSchedule.style.display = completion.scheduleDone ? 'flex' : 'none';
  if (badgeGoal) badgeGoal.style.display = completion.goalDone ? 'flex' : 'none';
  if (badgeMap) badgeMap.style.display = completion.mapDone ? 'flex' : 'none';
  if (btnNext) btnNext.style.display = completion.allDone ? 'block' : 'none';
  if (hint) hint.style.display = completion.allDone ? 'none' : 'block';
}
      ...
      area.innerHTML =
        '<div class="quiz-result-overlay">' +
          '<div class="quiz-result-card">' +
            '<div class="quiz-result-title">Твой результат</div>' +
...
function initLaptopHotspot() {
  ...
  document.getElementById('schedule-modal-ok').addEventListener('click', function() {
    ...
    localStorage.setItem(STORAGE_KEYS.laptopScheduleDone, '1');
    checkAllDone();
  });
...
function initZlataCard17() {
  initDoubleActionCard({
    cardId: 'zlata-card-17',
    wrapperId: 'zlata-card-wrapper-17',
    frontBtnId: 'inv-bizcard-17',
    backBtnId: 'inv-bizcard-17-back',
    nextScreenId: 'screen-19'
  });
}

function initZlataCard19() {
  initDoubleActionCard({
    cardId: 'zlata-card-19',
    wrapperId: 'zlata-card-wrapper-19',
    frontBtnId: 'inv-phone-19',
    backBtnId: 'inv-phone-19-back',
    nextScreenId: 'screen-20'
  });
}
function initZlataCard21() {
  initDoubleActionCard({
    cardId: 'zlata-card-21',
    wrapperId: 'zlata-card-wrapper-21',
    frontBtnId: 'inv-bag-21',
    backBtnId: 'inv-bag-21-back',
    nextScreenId: 'screen-21-1'
  });
}

function initZlataCard211() {
  initDoubleActionCard({
    cardId: 'zlata-card-211',
    wrapperId: 'zlata-card-wrapper-211',
    frontBtnId: 'inv-map-211',
    backBtnId: 'inv-map-211-back',
    nextScreenId: 'screen-21-2'
  });
}

function initDoubleActionCard(config) {
  var card = document.getElementById(config.cardId);
  var wrapper = document.getElementById(config.wrapperId);
  var btnFront = document.getElementById(config.frontBtnId);
  var btnBack = document.getElementById(config.backBtnId);

  if (!card || !wrapper || !btnFront || !btnBack) return;

  wrapper.addEventListener('click', function(e) {
    if (e.target.closest('#' + config.frontBtnId) || e.target.closest('#' + config.backBtnId)) return;
    card.classList.toggle('flipped');
  });

  function openNextScreen(e) {
    e.stopPropagation();
    showScreen(config.nextScreenId);
  }

  btnFront.addEventListener('click', openNextScreen);
  btnBack.addEventListener('click', openNextScreen);
}
...
var btnVenueNext = document.getElementById('btn-venue-next');
  if (btnVenueNext) {
    ...
    btnVenueNext.addEventListener('click', function() {
      localStorage.setItem(STORAGE_KEYS.mapViewed, '1');
      showScreen('screen-10');
    });
  }
}
